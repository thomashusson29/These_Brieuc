...
# Préparation pour analyse de sensibilité (exclusion des douteux)
df_sensi <- df %>% filter(Gold_Standard != "Douteux")
```
...
Ce que je propose : en soi, le nombre de sujets total requis est atteint (69 patients, mieux que 68), toutefois le nombre de sujets NÉGATIFS n'est pas atteint à cause d'une petite imprécision sur l'estimation de la prévalence *a priori*. Elle est de 60 % dans le protocole, mais de `r paste0(round(effectif_n_pos_main / effectif_n_total_main * 100, 1), " %")` dans la cohorte analysée. Comme l'étude vise une spécificité cible, le nombre de sujets négatifs est plus déterminant que le nombre total de sujets pour atteindre la précision visée.
...
ggplot(score_contribution_shapley, aes(x = Seuil, y = Gain_Se, fill = Test)) +
    ...
    theme_minimal() +
    labs(x = "Seuil du score", y = "Gain de Sensibilité (ΔSe)",
...
# Données extraites de la table 29
threshold_data <- data.frame(
    Seuil = c(1, 2, 3, 4),
    Sensibilité = c(0.956, 0.867, 0.667, 0.267),
    Spécificité = c(0.500, 0.625, 0.667, 0.792)
...
prev_max_pour_27 = 1 - (27 / n_tot_obs)
perte_prec_theo = ( (0.9 - qbeta(0.05, 24*0.9, 24*0.1+1)) - (0.9 - qbeta(0.05, 27*0.9, 27*0.1+1)) )
...
## Analyses de sensibilité : "Gold Standard fort"
...
comp_sensi_a1 <- data.frame(
  ...
  `Se GS princ.` = c(u1_performance_summary$Se, u2a_performance_summary$Se, u2b_performance_summary$Se, u3_performance_summary$Se),
...
**Tous les patients** n'ayant pas un Gold Standard fort positif sont assignés au groupe **Négatif**. Ce groupe inclut désormais les vrais négatifs initiaux, mais également les patients avec un examen/IRM positif qui n'ont pas eu d'infiltration ou dont l'infiltration n'a pas soulagé la douleur.
...
Dans cette approche, la modification du groupe de contrôle a un impact important. La sensibilité évaluée reste la même que dans l'approche 1 (puisque les cas positifs sont les mêmes), mais **la spécificité chute**.

Le groupe "Négatif" inclut désormais :

-   Les vrais malades cliniques (GS principal positif) qui rendent les tests positifs (Vrais Positifs cliniques), mais qui sont ici comptés artificiellement comme des Faux Positifs car ils n'ont pas eu d'infiltration. 

-   Les vrais négatifs initiaux (GS principal négatif) qui rendent les tests négatifs (Vrais Négatifs).

**La comparaison de la spécificité a donc un intérêt clinique très limité ici.**
...
comp_spe_a2 <- data.frame(
  Test = c("ULNT1", "ULNT2a", "ULNT2b", "ULNT3"),
  `Sp GS princ.` = c(u1_performance_summary$Sp, u2a_performance_summary$Sp, u2b_performance_summary$Sp, u3_performance_summary$Sp),
...
\newpage
# 2e partie : avec le nouveau fichier du 09-08-2926

```{r}
#read /Users/thomashusson/Documents/Projets/These_Brieuc/recueil_dernier_avec_sexe.csv dans df_2
df_2 <- read.csv("/Users/thomashusson/Documents/Projets/These_Brieuc/recueil_dernier_avec_sexe.csv", header = TRUE, sep = ",")
```
