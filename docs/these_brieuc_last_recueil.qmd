...
parse_excel_date <- function(x) {
    ...
    parsed <- lubridate::ymd(x_chr[is_text_date], quiet = TRUE)
    missing <- is.na(parsed)
    parsed[missing] <- lubridate::mdy(x_chr[is_text_date][missing], quiet = TRUE)
    missing <- is.na(parsed)
    parsed[missing] <- lubridate::dmy(x_chr[is_text_date][missing], quiet = TRUE)
...
roc_score <- roc(
    response = as.numeric(score_data$Gold_Standard_main == "Positif"),
