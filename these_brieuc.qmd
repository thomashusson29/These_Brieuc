...
2.  **Une perte de précision minime** : Si la spécificité avait été de 0.90, le passage de 27 à 24 sujets n'aurait fait perdre que `r round(perte_prec_theo * 100, 1)`% de précision sur la borne inférieure de l'intervalle de confiance.
3.  **La réalité de la performance** : La difficulté à atteindre la précision cible vient moins du nombre de sujets que de la performance réelle des tests. Avec une spécificité observée de `r sp_obs`, il aurait fallu `r n_neg_requis_sp_obs` sujets négatifs (soit environ `r n_total_requis_sp_obs` patients au total) pour stabiliser l'estimation avec la marge de 0.15 initialement souhaitée.
