...
Pour interpréter ces résultats, on peut raisonner **par addition** (approche de type Shapley) : on calcule la **contribution moyenne** de chaque test au gain de sensibilité du score global.

```{r}
#| label: score-contribution-plot
#| echo: false
...
ggplot(score_contribution_shapley, aes(x = Seuil, y = Gain_Se, fill = Test)) +
    ...
    theme_minimal() +
    labs(x = "Seuil du score (Nombre de tests positifs)", y = "Gain de Sensibilité cumulé",
         title = "Décomposition de la sensibilité du score")
```

-   Ce graphique montre clairement que l'`ULNT1` (en vert foncé) est le principal contributeur à la sensibilité du score, quel que soit le seuil choisi. Ajouter les autres tests augmente la spécificité mais au prix d'une perte importante de sensibilité.
...
## Analyse de sensibilité : exclusion des "Douteux"

Dans l'analyse principale, les patients ayant un Gold Standard "Douteux" ont été classés comme négatifs. Nous vérifions ici si leur exclusion pure et simple modifie les résultats.

```{r}
#| label: sensitivity-doubtful
#| echo: false

se_sensi <- sum(df_sensi$U1_E1 == "Positif" & df_sensi$Gold_Standard == "Positif") / sum(df_sensi$Gold_Standard == "Positif")
sp_sensi <- sum(df_sensi$U1_E1 == "Negatif" & df_sensi$Gold_Standard == "Negatif") / sum(df_sensi$Gold_Standard == "Negatif")

sensi_df <- data.frame(
    Metrique = c("Sensibilité (U1)", "Spécificité (U1)"),
    Analyse_Principale = c(u1_Se, u1_Sp),
    Exclusion_Douteux = c(se_sensi, sp_sensi)
)

kable(sensi_df, digits = 3, caption = "Comparaison avec et sans les patients 'Douteux'")
```

-   Les résultats sont **très proches**, ce qui suggère que le choix de codage des patients "Douteux" n'introduit pas de biais majeur dans l'évaluation des tests ULNT.
