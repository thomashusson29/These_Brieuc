...
results_by_level_status_plot <- ggplot(level_status_long, aes(x = Niveau, y = Effectif, fill = Statut_NCB)) +
    ...
    contrast_pattern_scale(c("NCB negatif", "NCB positif"), title = "Gold standard") +
...
results_by_level_test_metric_plot <- ggplot(level_test_metric_plot_data, aes(x = Niveau, y = Estimate, color = Metric, linetype = Metric, shape = Metric, group = Metric)) +
    ...
    scale_color_manual(values = if (figures_monochrome) c(Sensibilite = "black", Specificite = "black") else c(Sensibilite = nord("aurora")[4], Specificite = nord("frost")[4])) +
    scale_linetype_manual(values = c(Sensibilite = "solid", Specificite = "dashed")) +
    scale_shape_manual(values = c(Sensibilite = 16, Specificite = 17)) +
    scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.2)) +
    theme_minimal() +
    labs(
...
plot(roc_u1, col = roc_colours[1], lwd = 2, lty = roc_linetypes[1], main = "Courbes ROC comparées")
...
ggplot(threshold_data, aes(x = Seuil, y = Val, color = Metric, linetype = Metric, shape = Metric, group = Metric)) +
    ...
    scale_color_manual(values = if (figures_monochrome) c("Sensibilité" = "black", "Spécificité" = "black") else c("Sensibilité" = nord("aurora")[4], "Spécificité" = nord("frost")[4])) +
    scale_linetype_manual(values = c("Sensibilité" = "solid", "Spécificité" = "dashed")) +
    scale_shape_manual(values = c("Sensibilité" = 16, "Spécificité" = 17)) +
    theme_minimal() +
    labs(x = "Nombre de tests positifs requis (Seuil)", y = "Performance", color = "", linetype = "", shape = "",
...
ggplot(df_plot_pos, aes(x = Niveau, y = Effectif, fill = Niveau)) +
    ...
    contrast_pattern_scale(level_levels, title = "Niveau") +
    guides(pattern = "none") +
    theme_minimal() +
...
for (lvl in level_levels) {
    p <- ggplot(filter(level_test_perf, Niveau == lvl), aes(x = Test, y = Effectif, fill = Cellule)) +
        ...
        contrast_pattern_scale(
            c("VP", "FN", "FP", "VN"),
            title = "Résultat",
            labels = c(VP = "Vrai positif (VP)", FN = "Faux négatif (FN)", FP = "Faux positif (FP)", VN = "Vrai négatif (VN)")
        ) +
...
ggplot(level_grouped_long, aes(x = Niveau_groupe, y = Val, fill = Metric)) +
    ...
    scale_fill_manual(values = contrast_values(c("Se_U1", "Sp_U1"), c("Se_U1" = nord("aurora")[4], "Sp_U1" = nord("frost")[2])),
                      labels = c("Sensibilité", "Spécificité")) +
    contrast_pattern_scale(c("Se_U1", "Sp_U1"), title = "", labels = c("Sensibilité", "Spécificité")) +
    theme_minimal() +
