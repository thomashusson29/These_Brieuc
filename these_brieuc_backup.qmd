...
df_baseline <- df_baseline %>%
    ...
    mutate(
        Tabac = factor(Tabac, levels = c(0, 1), labels = c("Non", "Oui")),
        Infiltrations = factor(Infiltrations, levels = c(0, 1), labels = c("Non", "Oui")),
        Resultat_Inf = factor(Resultat_Inf, levels = c(0, 1), labels = c("Negatif", "Positif")),
        Gold_Standard = factor(Gold_Standard, levels = c(0, 1), labels = c("Negatif/Douteux", "Positif")),
        U1_E1 = factor(U1_E1, levels = c(0, 1), labels = c("Negatif", "Positif")),
        U2a_E1 = factor(U2a_E1, levels = c(0, 1), labels = c("Negatif", "Positif")),
        U2b_E1 = factor(U2b_E1, levels = c(0, 1), labels = c("Negatif", "Positif")),
        U3_E1 = factor(U3_E1, levels = c(0, 1), labels = c("Negatif", "Positif"))
    )

baseline_tbl <- df_baseline %>%
    select(Age, BMI, Sexe, Cote, Niveau, Tabac, Infiltrations, Resultat_Inf, Gold_Standard, U1_E1, U2a_E1, U2b_E1, U3_E1) %>%
    tbl_summary(
        missing = "no",
        label = list(
            Age = "Âge (années)",
...
u1_descriptive_table
u1_contingency_table
u1_performance_summary_table
```
...
u2a_descriptive_table
u2a_contingency_table
u2a_performance_summary_table
```
...
u2b_performance <- data.frame(
    ...
    VN = u2b_VN,
    Se = round(u2b_Se, 3),
    Se_IC95 = paste0("[", round(u2b_Se_IC[1], 3), " ; ", round(u2b_Se_IC[2], 3), "]"),
    Sp = round(u2b_Sp, 3),
    Sp_IC95 = paste0("[", round(u2b_Sp_IC[1], 3), " ; ", round(u2b_Sp_IC[2], 3), "]"),
    VPP = round(u2b_VPP, 3),
    VPP_IC95 = paste0("[", round(u2b_VPP_IC[1], 3), " ; ", round(u2b_VPP_IC[2], 3), "]"),
    VPN = round(u2b_VPN, 3),
    VPN_IC95 = paste0("[", round(u2b_VPN_IC[1], 3), " ; ", round(u2b_VPN_IC[2], 3), "]"),
    LR_pos = round(u2b_LR_pos, 2),
    LR_neg = round(u2b_LR_neg, 2),
    Youden = round(u2b_Youden, 3),
    AUC = round(u2b_AUC, 3),
    AUC_IC95 = paste0("[", round(u2b_AUC_IC[1], 3), " ; ", round(u2b_AUC_IC[3], 3), "]"),
    Fisher_p = signif(u2b_Fisher_p, 3)
)

u2b_performance_table <- kable(
    u2b_performance,
    format = if (knitr::is_latex_output()) {
        "latex"
    } else if (knitr::is_html_output()) {
    ...
    booktabs = TRUE,
    align = "c",
    caption = "Diagnostic performance of U2b compared with the reference standard",
    col.names = c(
        "Test", "N", "VP", "FN", "FP", "VN", "Se", "Se 95% CI", "Sp", "Sp 95% CI",
        "PPV", "PPV 95% CI", "NPV", "NPV 95% CI", "LR+", "LR-", "Youden", "AUC", "AUC 95% CI", "Fisher p"
    )
...
if (knitr::is_latex_output()) {
    u2b_performance_table <- u2b_performance_table %>%
        kable_styling(latex_options = c("hold_position", "scale_down"), full_width = FALSE, font_size = 7)
} else if (knitr::is_html_output()) {
    u2b_performance_table <- u2b_performance_table %>%
        kable_styling(full_width = FALSE, position = "center")
...
-   L'échantillon total comprend `r n_total_patients` patients.

-   Le gold standard brut est :
...
**Ce qu'on cherche.** Cette section décrit **chez quels patients** les résultats ont été obtenus. C'est important pour savoir à qui les conclusions du rapport peuvent raisonnablement s'appliquer.

**Comment lire ce résultat.** Il faut surtout regarder l'effectif total, la répartition du gold standard, et la distribution générale des niveaux métamériques. Sans cela, on risque de surinterpréter des résultats obtenus sur trop peu de patients ou dans une population trop particulière.

**Ce que montrent les données ici.** L'échantillon total comprend `69` patients. Le gold standard est renseigné comme `Positif` chez `45` patients et `Negatif` chez `24`. Les niveaux `C6` et `C7` sont les plus représentés, ce qui rend les analyses plus informatives à ces niveaux qu'en `C4` ou `C5`.

**Application pratique.** Les résultats du rapport s'appliquent surtout à une population de consultation spécialisée avec suspicion de NCB, et ils doivent être lus à partir des `69` patients réellement analysés. Cela explique aussi pourquoi certaines analyses par niveau restent fragiles malgré une base désormais complète sur le gold standard principal.
:::
...
::: {.callout-note icon=false title="Lecture simple"}
**Ce qu'on cherche.** Ici, on veut savoir si `U1` aide à repérer une NCB quand elle est présente et à faire baisser l'hypothèse de NCB quand il est négatif.

**Comment lire ce résultat.** La sensibilité dit, parmi les patients qui ont vraiment une NCB, combien ont un test positif. La spécificité dit, parmi ceux qui n'ont pas de NCB, combien gardent un test négatif. Les `LR+` et `LR-` sont particulièrement utiles en pratique : `LR+` indique ce qu'apporte un test positif, `LR-` ce qu'apporte un test négatif. Le `VPP` et le `VPN` décrivent ce qui se passe dans **cette cohorte** et dépendent de sa prévalence.

**Ce que montrent les données ici.** `U1` est le meilleur test isolé de la cohorte, avec `Se 0.864`, `Sp 0.60`, `VPP 0.792`, `VPN 0.714`, `LR+ 2.16`, `LR- 0.23`, `Youden 0.464` et `AUC 0.732`. Concrètement, il repère bien les patients avec NCB et il manque relativement peu de cas, mais il produit encore un nombre non négligeable de faux positifs. Son point fort est surtout le `LR- = 0.23`, qui fait baisser utilement la probabilité de NCB quand le test est négatif. L'AUC reste ici une mesure secondaire, car le test est binaire.

**Application pratique.** `U1` est un bon test d'orientation initial. Un `U1` négatif rend la NCB moins probable et peut aider à réévaluer le diagnostic, mais il ne suffit pas à lui seul pour exclure formellement une NCB. Un `U1` positif apporte un argument en faveur du diagnostic, sans être assez spécifique pour confirmer seul.
:::
...
::: {.callout-note icon=false title="Lecture simple"}
**Ce qu'on cherche.** Ici, on cherche à savoir si `U2a` peut jouer le même rôle que `U1`, c'est-à-dire détecter une partie importante des NCB tout en limitant les erreurs chez les patients sans NCB.

**Comment lire ce résultat.** Il faut surtout regarder le compromis entre sensibilité et spécificité, puis les rapports de vraisemblance. Plus le `LR+` est élevé, plus un positif renforce l'hypothèse de NCB. Plus le `LR-` est proche de `0`, plus un négatif aide à écarter la maladie.

**Ce que montrent les données ici.** `U2a` a `Se 0.795`, `Sp 0.56`, `VPP 0.761`, `VPN 0.609`, `LR+ 1.81`, `LR- 0.37`, `Youden 0.355` et `AUC 0.678`. Il reste plutôt sensible, donc il détecte encore une part importante des cas, mais il est moins convaincant que `U1`. Sa spécificité est modeste, ce qui veut dire qu'il garde un nombre notable de faux positifs. Son `LR-` aide un peu quand le test est négatif, mais clairement moins que celui de `U1`.

**Application pratique.** `U2a` est utile comme argument supplémentaire dans le raisonnement clinique, mais il paraît moins solide que `U1` pour servir de pivot. En pratique, il complète bien l'examen, sans justifier à lui seul une décision diagnostique.
:::
...
::: {.callout-note icon=false title="Lecture simple"}
**Ce qu'on cherche.** Pour `U2b`, la question est de savoir s'il offre un meilleur équilibre entre capacité à détecter les NCB et capacité à éviter des faux positifs.

**Comment lire ce résultat.** Un test plus équilibré a une sensibilité et une spécificité moins déséquilibrées, mais cela ne suffit pas : il faut aussi regarder ce que devient la probabilité de NCB après un positif (`LR+`) et après un négatif (`LR-`).

**Ce que montrent les données ici.** `U2b` a `Se 0.682`, `Sp 0.64`, `VPP 0.769`, `VPN 0.533`, `LR+ 1.89`, `LR- 0.50`, `Youden 0.322` et `AUC 0.661`. Son profil est un peu plus équilibré entre sensibilité et spécificité que celui de `U2a`, mais il reste moins bon que `U1` pour ne pas manquer les cas. Un résultat positif est un peu plus crédible qu'avec `U2a`, mais le gain reste modeste. À l'inverse, un résultat négatif aide moins à faire baisser la probabilité de NCB que pour `U1`.

**Application pratique.** `U2b` a un intérêt comme test de complément, surtout si l'on cherche un profil un peu moins centré sur la sensibilité pure. En revanche, il n'apparaît pas ici comme le meilleur compromis global de la cohorte.
:::
...
::: {.callout-note icon=false title="Lecture simple"}
**Ce qu'on cherche.** Ici, on regarde si `U3` apporte une information diagnostique utile, soit pour repérer les NCB, soit pour rassurer quand il est négatif.

**Comment lire ce résultat.** Un test utile doit idéalement garder une bonne sensibilité, une spécificité correcte, et surtout des rapports de vraisemblance qui modifient réellement la probabilité clinique de NCB.

**Ce que montrent les données ici.** `U3` a `Se 0.386`, `Sp 0.68`, `VPP 0.68`, `VPN 0.386`, `LR+ 1.21`, `LR- 0.90`, `Youden 0.066` et `AUC 0.533`. Cela veut dire qu'il manque beaucoup de patients réellement atteints et qu'un résultat négatif aide très peu à écarter la NCB. Son résultat positif renforce aussi faiblement l'hypothèse diagnostique. Sur le plan statistique, l'association avec le gold standard est ici faible, contrairement à `U1`, `U2a` et `U2b`.

**Application pratique.** Dans cette cohorte, `U3` a un faible intérêt isolé. Il paraît nettement moins utile que `U1`, `U2a` ou `U2b` si l'objectif est d'orienter concrètement la décision clinique.
:::
